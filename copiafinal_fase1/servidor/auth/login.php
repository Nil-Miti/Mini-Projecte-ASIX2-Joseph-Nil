...
if (isset($_REQUEST['login'])) {
    ...
    $sql = "SELECT * FROM usuario WHERE USER_NAME = '$name' OR email = '$email'";
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        if (password_verify($password, $row['PASSW'])) {
            if ($row['estado'] == 'autorizado') {
                $_SESSION['id_usuario'] = 1;  // Ajusta esto según tu estructura de base de datos
                ...
                } elseif ($row['estado'] == 'no_autorizado') {
